...
template<typename T> my_array_t<T>::my_array_t(): my_array(){ __login__( std::clog, this, "my_array_t<T>", "my_array_t()" ); }
template<typename T> my_array_t<T>::my_array_t( my_array_t<T> const & v ): my_array( v )
{
	__login__( std::clog, this, "my_array_t<T>", "my_array_t( my_array_t<T> const & )" );
	this->copy( v );
}
template<typename T> my_array_t<T>::~my_array_t()
{
	__login__( std::clog, this, "my_array_t<T>", "~my_array_t()" );
	const size_type t_size = this->size();
	pointer p = this->data();
	for(int i = 0; i < t_size; ++i, ++p )
		p->~T();
