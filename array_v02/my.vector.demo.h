...
int my_vector_t_demo( char * arg )
{
	...
	for(int i = 0; i < a1.size(); ++i )
		a1[i] = 2 * i + 1;
	cout << "-------" << endl;
	my_vector_t<T> a2( a1 );// copy ctor [with T]
	print_v( a2, "a2" );

	cout << "-------" << endl;
	my_vector_t<int> b1( size, 7 );// ctor [with T = int]
	print_v( b1, "b1" );
	cout << "-------" << endl;
	my_vector_t<T> b2( b1 );// copy ctor [with T, with U = int]
	print_v( b2, "b2" );

	cout << "-------" << endl;
	my_vector_t<T> c1{ 1, 4, 9};
	print_v( c1, "c1" );
	cout << "-------" << endl;
	my_vector_t<int> c2{ 1, 4, 9};
	print_v( c2, "c2" );
...
int main( int argc, char ** argv )
{
	...
	return my_vector_t_demo<Foo<long double> >( argv[1] );
	//return my_vector_t_demo<long double>( argv[1] );
	//return my_vector_demo( argv[1] );
	//return my_vector_t_demo<uint32_t>( argv[1] );
	//return my_vector_demo<uint16_t>( argv[1] );
	//return my_vector_demo<uint8_t>( argv[1] );
}
